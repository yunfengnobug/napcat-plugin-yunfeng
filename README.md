...
7. **自定义 API**：消息按精确词 / 模糊词 / 正则触发 → 可串行多个外部接口 → 按模板拼话术 → 发到指定群/好友（群侧需开启该功能）
   ...
   - 每步可配超时（默认 8000ms）与预期返回值（最多 2 条，且/或；第 1 步用 `res1.xxx`，第 2 步用 `res2.xxx`）：value 可写 `{{id}}` 等变量；**留空**则仅要求该 key 存在即通过。「严格中止」默认开：只按**实际结果**拦截（超时 / 预期不符 / 话术变量缺失），请求模板缺变量按空串照常请求
